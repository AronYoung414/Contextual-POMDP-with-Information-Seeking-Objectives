...
class ContinuousLightDarkPOMDP:
    ...
    def obs_sigma(self, x, context):
        """
        Observation noise σ(x,c) for generative likelihood.
        Uses discontinuous light/dark boundary but evaluated at predicted x.
        """
        if context == 0:
            return self.light_noise if x > 0 else self.dark_noise
        else:
            return self.light_noise if x < 0 else self.dark_noise
